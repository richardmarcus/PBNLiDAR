...
for file in all_files:
    ...
    cv2.imwrite("mask.png", mask_image/num_files*255)
    k+=1
    #sleep
    time.sleep(0.001)
...
cv2.imwrite(path + "mask.png", mask_image*255)
#use base mask to filter the mask image

mask_image = mask_image * (1-base_mask)
cv2.imwrite(path + "filtered_mask.png", mask_image*255)

mask_image = mask_image > 0.2
mask_image = mask_image.astype(np.uint8)
cv2.imwrite(path + "advanced_mask.png", mask_image*255)
